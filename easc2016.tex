...
\cite{Khoury2013}. Solving the Poisson equation for the
...
or AMG\footnote{a reference to James, email him or ask Paul}. We explore both 
...
the physical case we study and the way it was initially performed, i.e. direct 
numerical simulation of pipe flow \cite{Khoury2013}.

\subsection{Numerical method}
The incompressible Navier-Stokes are given here by
...
\end{align}
where $\mathbf{u}$ is the velocity and $p$ the pressure. The Reynolds number $Re = \frac{U L}{\nu}$ is expressed as a function of a typical velocity scale $U$, length scale $L$ and kinematic viscosity $\nu$. Equations~\refeqn{eqn:NS_momentum} and~\refeqn{eqn:NS_continuity} are called the continuity and momentum equations respectively. There are two main solvers, called PNPN and PNPN-2, available within Nek5000 for computing the solution of the incompressible 
Navier-Stokes equations, and of these one is also amenable to non-divergence free flows, as available in \cite{Tomboulides1997}, namely PNPN. Although we operate in the incompressible
...
Equation \refeqn{eqn:hmhz_pres}, the Poisson equation for the pressure, is the main 
source of stiffness and its efficient resolution by an iterative solver is preceded 
by two steps. First of all, the pressure at each time iteration is projected onto a 
...
method (GMRES). Equation~\refeqn{eqn:hmhz_vel}, the Helmholtz equation for the velocity components, is solved using the Conjugate Gradient (CG) iterative solver with a simple Jacobi preconditioner.
% \begin{align}
...
important part of the work and communication. The Helmholtz equation is solved using the CG method for each componenf of the velocity.
\begin{algorithm}
\begin{algorithmic}
\Procedure{Solver}{}
...
for the test in Nek) is measured 
...
representing the tensor products
...
The initial stage is meant to allow for the higher order restart, proper initialization 
...
limit that would in most of our test runs never be observed. For Nek on Mira it has
...
The test case used to explore the scaling behavior of Nek is the one of a turbulent 
flow in a straight pipe, a generic and widely known case across the CFD community.
This should allow potential users to estimate and compare the scaling of Nek to other CFD software.
...
This limitation cannot be fully explained from the non-dimensional latencies and bandwidths from \reftab{tab:alpha_beta}. %, as we do not expect the non-dimensional latency $\alpha$ to have similar values on Titan and Mira. 
...
% Weak scaling
As a byproduct of our analysis we can mimic a weak scaling analysis by stacking up all problem sizes at every MPI rank count. On Mira the communication is mostly latency bound with a small influence from the
