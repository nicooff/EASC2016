...
Titan & Cray XK7 & $299,008$ & $16$ &1&3D torus& $???$ & $???$ &\\ % alpha*= 2.2515e-06 s, beta*= 1.4179e-09, t_a= ???, alpha=alpha*/t_a, beta_beta*/t_a
Beskow & Cray XC40 & $53,632$ & $32$ &1&DragonFly & $???$ & $???$ &\\ % alpha*=2.5486e-06 s, beta*=8.2542e-10 wd/s, t_a= ???, alpha=alpha*/t_a, beta=beta*/t_a
\hline
\end{tabular}
...
The momentum equation is time integrated via an Implicit-Explicit scheme, also known as BDFk-EXTk (Backward DiFferences and EXTrapolation of order k). We illustrate it semidiscretely, following \cite{Tomboulides1997}
...
In a similar way, Equation \refeqn{eqn:hmhz_vel}, the Poisson equation for the velocity, is solved using the conjugate gradient (CG) iterative solver but without the need for a Schwarz method or a coarse grid solve.
% \begin{align}
...
\label{sec:analysis}
We assume that large scale runtime performance is influenced by 
\begin{enumerate}
  ...
  \item partitioning imbalance.\label{enum:e}
\end{enumerate}
...
extensively described in \cite{fischer:scaling}. We therefore focus to extract
information on \ref{enum:b} to \ref{enum:e} from our conducted runs. Our
...
\begin{figure}
  \centering
  \subfigure[$Re_{\tau} = 180$]{
  \includegraphics[width=\linewidth]{./figures/beskow/scaling_ReTau180_beskow.pdf}
  }
  \subfigure[$Re_{\tau} = 360$]{
  \includegraphics[width=\linewidth]{./figures/beskow/scaling_ReTau360_beskow.pdf}
  }
  \subfigure[$Re_{\tau} = 550$]{
  \includegraphics[width=\linewidth]{./figures/beskow/scaling_ReTau550_beskow.pdf}
  }
  \subfigure[$Re_{\tau} = 1000$]{
  \includegraphics[width=\linewidth]{./figures/beskow/scaling_ReTau1000_beskow.pdf}
  }
\caption{Beskow}
