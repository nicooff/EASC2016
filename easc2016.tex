...
\maketitle
\begin{abstract}
In this paper, strong scaling and benchmarking of the high-order spectral element solver Nek5000 are performed. The previous extensive benchmarking for Nek5000 was done for terascale computers. This study updates the results to modern petascale computers. The test cases considered for the simulations correspond to a turbulent flow in a straight pipe at four different friction Reynolds numbers $Re_{\tau} = 180$, $360$, $550$ and $1000$. Results on various machines with various architectures and interconnect networks, namely Mira (IBM Blue Gene/Q), Beskow (Cray XC40) and Titan (Cray XK7) are studied. During the runs, the main blocks of the code are timed. Furthermore, sampling and profiling tools are used to measure the communication time over a large range of compute cores. A theoretical linear model for parallel communication is introduced and used to assess the observed weak and strong scaling behaviors of the code. We also study the effect of the two coarse grid solvers XXT and AMG on the computational time. Superlinear scaling due to a reduction in cache misses is observed on each computer. The strong scaling limit is attained for roughly $5000-10,000$ degrees of freedom per core on Mira, $30,000-50,0000$ on Beskow, with only a small impact of the problem size for both machines, and ranges between $10,000$ and $220,000$ depending on the problem size on Titan. This work aims to be a reference for Nek users and serve as a basis for the development of the code toward the advent of exascale supercomputers.
\end{abstract}
