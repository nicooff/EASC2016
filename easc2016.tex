...
\begin{algorithm}
\caption{Structure of Nek5000 using the PNPN method.}\label{alg:code_struct}
\begin{algorithmic}
\Procedure{Solver}{}
\For {k=1,...,nsteps}
\State $r_p \leftarrow \text{proj}_{X_{r_p}}(r_p)$
\State $p \leftarrow \text{Helmoltz}(H,r_p)$
\State $p \leftarrow \text{proj}_{X_p}(p)$
\State $r_u \leftarrow \text{proj}_{X_{r_u}}(r_u)$
\State $u \leftarrow \text{Helmoltz}(H,r_u)$
\State $u \leftarrow \text{proj}_{X_u}(u)$
\EndFor
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
\caption{Helmholtz solve.}\label{alg:code_struct}
\begin{algorithmic}
